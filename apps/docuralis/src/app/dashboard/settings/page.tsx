  ...
  const confirmPlanChange = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPlanMessage({
          type: 'error',
          text: error.error || t('plan.changeError'),
        })
      }
    } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <p className="font-medium">
                  {session?.user?.name || t('account.notSet')}
                </p>
              </div>
              <div>
                <Label className="text-sm text-gray-500">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              {planMessage && (
                <div
                  className={`p-4 rounded-lg ${planMessage.type === 'success' ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'}`}
                >
                  {planMessage.text}
                </div>
              )}
        ...
        <AlertDialog open={showPlanDialog} onOpenChange={setShowPlanDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t('plan.confirmChangeDesc')}{' '}
                {selectedPlan && t(`plan.${selectedPlan.toLowerCase()}`)}?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
