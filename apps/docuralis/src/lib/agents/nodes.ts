...
import type { AgentState, DocumentChunk, GradingResult, ReflexionGradingResult } from "./types";
import {
...
export async function decomposeQuery(state: AgentState): Promise<Partial<AgentState>> {
  try {
    const prompt = DECOMPOSE_QUERY_PROMPT.replace("{query}", state.query);

    const response = await model.invoke([
      { role: "user", content: "You are an expert at breaking down complex queries." },
...
export async function retrieveDocuments(state: AgentState): Promise<Partial<AgentState>> {
  try {
    ...
    for (const query of queriesToSearch) {
      ...
      if (state.multilingual || state.translatorMode) {
        const translatedQuery = await translateQuery(query, "Nederlands (Dutch)");
...
export async function gradeDocumentsClassical(
  ...
  try {
    ...
    for (const doc of state.retrievedDocs) {
      const prompt = BINARY_GRADING_PROMPT.replace("{systemMessage}", BINARY_GRADING_SYSTEM_PROMPT)
...
export async function gradeDocumentsReflexion(
  ...
  try {
    ...
    for (const doc of state.retrievedDocs) {
      ...
      try {
        const result = JSON.parse(response.content.toString()) as ReflexionGradingResult;
...
export async function generateResponse(state: AgentState): Promise<Partial<AgentState>> {
  try {
    ...
    const prompt = FINAL_RESPONSE_PROMPT.replace("{documents}", formattedDocs).replace(
      "{question}",
      state.query
    );
    ...
    if (answer.startsWith("```") && answer.endsWith("```")) {
      ...
      answer = answer.replace(/```markdown/g, "").replace(/```/g, "").trim();
    }
...
export async function translateQuery(query: string, targetLang: string): Promise<string> {
  try {
    const systemPrompt = TRANSLATE_QUERY_SYSTEM_PROMPT.replace("{targetLang}", targetLang);
