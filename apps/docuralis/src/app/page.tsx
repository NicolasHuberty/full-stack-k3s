...
import { Shield, Zap, FileText, Users, Database, Search, Globe, Bot, CheckCircle, Briefcase, Heart, Code, Smartphone } from 'lucide-react'
...
const getDemoExamples = (locale: string) => [
  {
    ...
    sources: [
      ...
      },
      {
        file: "Supplier_Agreement_2024.pdf",
        page: 8,
        excerpt: "5.3 Late Payment: Any payment not received by the due date will incur",
    ...
    sources: [
      {
        ...
        excerpt2: "when caring for an immediate family member with a serious health condition."
      },
      {
        file: "Employee_Handbook_2024.pdf",
        page: 24,
        ...
        excerpt2: ". Extended family leave requests may be approved at management discretion."
      }
    ]
  },
  {
    ...
    sources: [
      ...
      },
      {
        ...
        highlight: "X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset",
...
function DemoCard({ example, t }: { example: ReturnType<typeof getDemoExamples>[0], t: (key: string) => string }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 shadow-lg flex flex-col h-full">
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-accent">{t(example.categoryKey)}</span>
      </div>
      ...
      {step === 2 ? (
        <div className="space-y-3 flex-1 animate-in fade-in duration-500">
          ...
          <div className="rounded-lg border border-accent/30 bg-accent/5 p-3">
            ...
            <p className="text-xs leading-relaxed text-foreground" dangerouslySetInnerHTML={{ __html: example.answer }} />
          </div>

          {/* Sources */}
          <div className="space-y-1">
            <h3 className="text-xs font-semibold text-muted-foreground">{t('demo.sources')}</h3>
            {example.sources.slice(0, 1).map((source, idx) => (
              <div key={idx} className="rounded border border-border bg-background p-2">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">
                  {source.excerpt} <mark className="bg-accent/30 px-0.5">{source.highlight}</mark>{source.excerpt2}
                </p>
              </div>
            ))}
            {example.sources.length > 1 && (
              <p className="text-xs text-accent">+{example.sources.length - 1} more sources</p>
            )}
          </div>
        </div>
      ) : step === 1 ? (
...
function AIAgentsCatalog({ t }: { t: (key: string) => string }) {
  ...
  const fullAgentQuery = "Le juge des référés peut-il ordonner la passation d'un acte authentique de vente ?"
  ...
  const agents = [
    {
      ...
      description: 'Analyzes contracts, case law, and legal documents with expert precision',
      descriptionFr: 'Analyse les contrats, la jurisprudence et les documents juridiques avec une précision experte',
      descriptionNl: 'Analyseert contracten, rechtspraak en juridische documenten met expertprecisie',
      features: ['Contract Analysis', 'Jurisprudence Research', 'Legal Citations', 'Compliance Checks']
    },
    {
      id: 'hr',
      name: 'HR Specialist',
      ...
      description: 'Handles employee handbooks, policies, benefits, and HR regulations',
      descriptionFr: 'Gère les manuels employés, politiques, avantages et réglementations RH',
      descriptionNl: 'Behandelt werknemershandboeken, beleid, voordelen en HR-regelgeving',
      features: ['Policy Lookup', 'Benefits Guide', 'Leave Management', 'Onboarding']
    },
    {
      id: 'tech',
      name: 'Developer Docs',
      ...
      description: 'Navigates technical documentation, APIs, architecture, and code standards',
      descriptionFr: 'Navigate dans la documentation technique, APIs, architecture et standards de code',
      descriptionNl: 'Navigeert door technische documentatie, APIs, architectuur en codestandaarden',
      features: ['API References', 'Code Examples', 'Architecture Guides', 'Best Practices']
    },
    {
      id: 'finance',
      ...
      description: 'Analyzes financial reports, budgets, compliance, and accounting standards',
      descriptionFr: 'Analyse les rapports financiers, budgets, conformité et normes comptables',
      descriptionNl: 'Analyseert financiële rapporten, budgetten, compliance en boekhoudnormen',
      features: ['Report Analysis', 'Budget Planning', 'Tax Compliance', 'Audit Support']
    },
    {
      id: 'sales',
      ...
      description: 'Product specs, pricing, competitive analysis, and sales playbooks',
      descriptionFr: 'Spécifications produits, tarification, analyse concurrentielle et guides de vente',
      descriptionNl: 'Productspecificaties, prijzen, concurrentieanalyse en verkoophandboeken',
      features: ['Product Info', 'Pricing Lookup', 'Competitor Intel', 'Sales Scripts']
    },
    {
      id: 'compliance',
      ...
      description: 'GDPR, industry regulations, internal policies, and audit requirements',
      descriptionFr: 'RGPD, réglementations sectorielles, politiques internes et exigences d\'audit',
      descriptionNl: 'AVG, brancheregelgeving, intern beleid en auditvereisten',
      features: ['GDPR Checks', 'Regulation Updates', 'Policy Compliance', 'Audit Trails']
    }
  ]
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      } else if (agentStep === 0 && agentQuery.length === fullAgentQuery.length) {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 w-80 rounded-2xl ${agent.bgColor} border-2 ${
                    isSelected ? 'border-accent shadow-2xl scale-105' : 'border-border/50 hover:border-border'
                  } p-6 text-left transition-all duration-300 hover:shadow-xl group`}
                >
                  {/* Header with Icon and Badge */}
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-3 rounded-xl bg-gradient-to-br ${agent.gradient} shadow-lg group-hover:scale-110 transition-transform`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-border/50">
                    <div className={`text-xs font-semibold ${agent.color} flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity`}>
          ...
          {agents.map((agent) => (
            <div
              ...
              className={`h-1 rounded-full transition-all ${
                selectedAgent === agent.id ? 'w-8 bg-accent' : 'w-1 bg-muted-foreground/30'
              }`}
            />
          ))}
        </div>
      ...
      {selectedAgent === 'lawyer' && (
        <div className="rounded-lg border border-border bg-card p-6 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="mb-4 flex items-center gap-2 text-accent">
            ...
            <span className="font-semibold">Lawyer Agent - Legal Research Demo</span>
          </div>
            ...
            <div className="space-y-4 animate-in fade-in duration-500">
              ...
              <div className="rounded-lg border border-accent/30 bg-accent/5 p-6">
                ...
                <p className="text-sm leading-relaxed text-foreground mb-4">
                  Le juge des référés peut, sauf exceptions, ordonner l'exécution en nature d'une obligation, y compris la passation d'un acte authentique,
                  pour autant que <strong>le droit invoqué ne soit pas sérieusement contestable</strong> et en présence d'une <strong>situation d'urgence</strong>.
                  Cependant, en matière d'actes authentiques, une exception importante existe lorsque la loi impose une forme solennelle particulière
                  (actes authentiques notariés), ce qui limite la compétence du juge des référés.
                </p>

                <div className="space-y-3 text-sm">
                  <div>
                    <h4 className="font-semibold text-foreground mb-2">✓ Fondements et conditions</h4>
                    <ul className="space-y-2 text-muted-foreground ml-4">
                      <li>• Le juge des référés peut ordonner l'exécution forcée en nature d'un engagement</li>
                      <li>• Exemple: Bruxelles, 24 avril 1990 - réintégration et passation d'acte ordonnées</li>
                      <li>• Condition: droit non sérieusement contesté et urgence établie</li>
                    </ul>
                  </div>

                  <div>
                    <h4 className="font-semibold text-foreground mb-2">⚠️ Limites légales</h4>
                    <ul className="space-y-2 text-muted-foreground ml-4">
                      <li>• Le juge ne peut se substituer au notaire pour actes authentiques obligatoires</li>
                      <li>• Respect des formes solennelles imposées par la loi (ex: vente immobilière)</li>
                      <li>• Cassation: interdiction d'empiéter sur prérogatives notariales</li>
                    </ul>
                  </div>
                </div>
              </div>
                ...
                <div className="grid gap-2">
                  <div className="rounded-lg border border-border bg-background p-3 hover:border-accent/50 transition">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">vancompernolle-proceduresrefere-1998.pdf</span>
                      </div>
                      <span className="text-xs text-muted-foreground">p. 156</span>
                    </div>
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Le juge peut ordonner <mark className="bg-accent/30 px-1">la passation en nature d'un acte authentique</mark>
                      non sérieusement contesté. Bruxelles, 24 avril 1990, J.L.M.B., 1991, p. 98.
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-border bg-background p-3 hover:border-accent/50 transition">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4 text-accent" />
                        <span className="text-sm font-medium">dirix-beslagexecutierecht-2001.pdf</span>
                      </div>
                      <span className="text-xs text-muted-foreground">p. 234</span>
                    </div>
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Limites: <mark className="bg-accent/30 px-1">le juge ne peut remplacer un acte notarié obligatoire</mark>
                      par son jugement, sauf dérogation légale expresse. Respect des compétences notariales.
                    </p>
                  </div>
                  ...
                  <div className="rounded-lg border border-border bg-background p-3 hover:border-accent/50 transition">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4 text-accent" />
                        <span className="text-sm font-medium">1999-059.pdf</span>
                      </div>
                      <span className="text-xs text-muted-foreground">p. 12</span>
                    </div>
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      Le juge des référés peut <mark className="bg-accent/30 px-1">condamner à l'exécution forcée d'une obligation</mark>,
                      y compris la passation d'acte authentique, sous conditions d'urgence et de clarté du droit.
                    </p>
                  </div>
                </div>
      ...
      {selectedAgent && selectedAgent !== 'lawyer' && (
        <div className="rounded-lg border border-border bg-card p-12 text-center">
          ...
          <p className="text-muted-foreground">
            ...
            {selectedAgent === 'tech' && 'Tech Documentation agent demo coming soon...'}
          </p>
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center">
              <Image src="/docuralis.png" alt="Docuralis" width={132} height={132} className="h-13 w-auto" />
            </div>
            <div className="hidden md:flex items-center space-x-8">
              <a href="#features" className="text-muted-foreground hover:text-foreground transition">{t('nav.features')}</a>
              <a href="#agents" className="text-muted-foreground hover:text-foreground transition">{t('nav.agents')}</a>
              <a href="#pricing" className="text-muted-foreground hover:text-foreground transition">{t('nav.pricing')}</a>
              <Link href="/login" className="text-muted-foreground hover:text-foreground transition">{t('nav.login')}</Link>
              ...
              <Link href="/register" className="rounded-md bg-primary px-4 py-2 text-primary-foreground hover:opacity-90 transition">
                {t('nav.signup')}
              </Link>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <h1 className="text-5xl font-bold tracking-tight text-foreground sm:text-6xl">
              ...
              <span className="block text-accent">{t('hero.title').split(', ')[1]}</span>
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
              {t('hero.subtitle')}
            </p>
            <div className="mt-10 flex items-center justify-center gap-x-6">
              <Link href="/register" className="rounded-md bg-primary px-6 py-3 text-lg font-semibold text-primary-foreground hover:opacity-90 transition">
                {t('hero.cta')}
              </Link>
            </div>
          </div>
          ...
          <div className="mx-auto mt-16 max-w-7xl">
            <h2 className="text-center text-2xl font-bold mb-8 text-foreground">{t('demo.title')}</h2>
        ...
        <div className="mx-auto max-w-7xl text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">{t('compliance.badges.gdpr')}</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="h-5 w-5 text-secondary" />
              <span className="font-semibold">{t('compliance.badges.aiAct')}</span>
            </div>
            <div className="flex items-center gap-2">
              <Database className="h-5 w-5 text-secondary" />
              <span className="font-semibold">{t('compliance.badges.servers')}</span>
            </div>
            <div className="flex items-center gap-2">
              <Globe className="h-5 w-5 text-secondary" />
              <span className="font-semibold">{t('compliance.badges.multilingual')}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-lg border border-border bg-card p-6">
              ...
              <h3 className="mt-4 text-xl font-semibold">{t('features.rag.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.rag.description')}</p>
            </div>

            <div className="rounded-lg border border-border bg-card p-6">
              <Shield className="h-10 w-10 text-primary" />
              <h3 className="mt-4 text-xl font-semibold">{t('features.gdpr.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.gdpr.description')}</p>
            </div>

            <div className="rounded-lg border border-border bg-card p-6">
              <Bot className="h-10 w-10 text-accent" />
              <h3 className="mt-4 text-xl font-semibold">{t('features.agents.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.agents.description')}</p>
            </div>

            <div className="rounded-lg border border-border bg-card p-6">
              <FileText className="h-10 w-10 text-secondary" />
              <h3 className="mt-4 text-xl font-semibold">{t('features.sources.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.sources.description')}</p>
            </div>

            <div className="rounded-lg border border-border bg-card p-6">
              <Database className="h-10 w-10 text-primary" />
              <h3 className="mt-4 text-xl font-semibold">{t('features.data.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.data.description')}</p>
            </div>

            <div className="rounded-lg border border-border bg-card p-6">
              <Globe className="h-10 w-10 text-accent" />
              <h3 className="mt-4 text-xl font-semibold">{t('features.multilingual.title')}</h3>
              <p className="mt-2 text-muted-foreground">{t('features.multilingual.description')}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid gap-6 md:grid-cols-3">
            <div className="rounded-lg border border-accent/30 bg-accent/5 p-6 text-center">
              ...
              <h3 className="font-semibold mb-2">{t('aiAgents.superpowers.understanding.title')}</h3>
              <p className="text-sm text-muted-foreground">{t('aiAgents.superpowers.understanding.description')}</p>
            </div>
            <div className="rounded-lg border border-secondary/30 bg-secondary/5 p-6 text-center">
              ...
              <h3 className="font-semibold mb-2">{t('aiAgents.superpowers.cited.title')}</h3>
              <p className="text-sm text-muted-foreground">{t('aiAgents.superpowers.cited.description')}</p>
            </div>
            <div className="rounded-lg border border-primary/30 bg-primary/5 p-6 text-center">
              <Bot className="h-8 w-8 mx-auto mb-3 text-primary" />
              <h3 className="font-semibold mb-2">{t('aiAgents.superpowers.expertise.title')}</h3>
              <p className="text-sm text-muted-foreground">{t('aiAgents.superpowers.expertise.description')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 py-20 sm:px-6 lg:px-8 bg-gradient-to-b from-muted/30 to-background">
        <div className="mx-auto max-w-7xl">
          <div className="text-center mb-16">
            ...
            <p className="mx-auto max-w-3xl text-lg text-muted-foreground">
              {messages?.teamManagement?.subtitle || 'Collaborate seamlessly with powerful team management and shared knowledge bases.'}
            </p>
          </div>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
              <div className="flex-1 order-2 lg:order-1">
                ...
                <p className="text-lg text-muted-foreground mb-6">
                  {messages?.teamManagement?.teams?.description || 'Create workspaces, invite members, assign roles, and collaborate on shared documents'}
                </p>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-primary flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Invitez des membres par email avec rôles personnalisés' : locale === 'nl' ? 'Nodig leden uit via email met aangepaste rollen' : 'Invite members via email with custom roles'}
                    </span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-primary flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Admin, Éditeur, Lecteur - contrôlez chaque niveau d\'accès' : locale === 'nl' ? 'Admin, Editor, Reader - beheer elk toegangsniveau' : 'Admin, Editor, Reader - control every access level'}
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">1,234 documents • Team access</p>
                      </div>
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">567 documents • Private</p>
                      </div>
                      <Shield className="h-4 w-4 text-primary" />
                    </div>
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border opacity-60">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-semibold">Tech Documentation</p>
                        <p className="text-xs text-muted-foreground">890 documents • Shared</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 order-2">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-2xl font-bold text-foreground">
                    {messages?.teamManagement?.rags?.title || 'Shared Knowledge Bases'}
                  </h3>
                </div>
                <p className="text-lg text-muted-foreground mb-6">
                  {messages?.teamManagement?.rags?.description || 'Build organization-wide RAG collections accessible to your entire team with permission controls'}
                </p>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-secondary flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Collections publiques ou privées par équipe' : locale === 'nl' ? 'Publieke of privé collecties per team' : 'Public or private collections per team'}
                    </span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-secondary flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Synchronisation automatique des documents' : locale === 'nl' ? 'Automatische synchronisatie van documenten' : 'Automatic document synchronization'}
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
              <div className="flex-1 order-2 lg:order-1">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-2xl font-bold text-foreground">
                    {messages?.teamManagement?.access?.title || 'Granular Access Control'}
                  </h3>
                </div>
                <p className="text-lg text-muted-foreground mb-6">
                  {messages?.teamManagement?.access?.description || 'Define who can view, edit, or manage each knowledge base with role-based permissions'}
                </p>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-accent flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Permissions par base de connaissances' : locale === 'nl' ? 'Rechten per kennisbank' : 'Per-knowledge-base permissions'}
                    </span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-accent flex-shrink-0 mt-0.5" />
                    <span className="text-muted-foreground">
                      {locale === 'fr' ? 'Audit trail de toutes les actions' : locale === 'nl' ? 'Audit trail van alle acties' : 'Audit trail of all actions'}
                    </span>
                  </li>
                </ul>
              </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-card/50 backdrop-blur-sm border border-border rounded-2xl p-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-muted-foreground mb-4 max-w-xl">
                {messages?.mobileApp?.subtitle || 'Download our mobile app for instant access to your AI agents anywhere, anytime.'}
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center md:justify-start items-center">
                ...
                <span className="text-xs text-muted-foreground">
                  {messages?.mobileApp?.platforms || 'Available on iOS and Android'}
                </span>
              </div>
            </div>
              ...
              <div className="relative w-[220px] h-[440px] rounded-[2.5rem] border-[6px] border-foreground/20 bg-background shadow-2xl overflow-hidden">
                <div className="p-3 h-full flex flex-col bg-gradient-to-b from-background via-primary/5 to-accent/5">
                  ...
                  <div className="flex items-center justify-between mb-3 pb-2 border-b border-border/30">
                    <div className="flex items-center gap-2">
                      <Image src="/docuralis.png" alt="App" width={28} height={28} />
                  ...
                  <div className="mb-2 flex items-center justify-between bg-muted/30 rounded-lg px-2 py-1.5 border border-border/40">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[8px] text-muted-foreground font-medium">
                        {locale === 'fr' ? '24 conversations' : locale === 'nl' ? '24 gesprekken' : '24 chats'}
                      </span>
                    </div>
                    <span className="text-[7px] text-accent">
                      {locale === 'fr' ? 'Voir tout' : locale === 'nl' ? 'Bekijk alles' : 'View all'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 flex-1 overflow-hidden">
                    ...
                    <div className="bg-primary/10 border border-primary/30 rounded-lg p-2">
                      <p className="text-[9px] font-semibold text-primary mb-0.5">
                        {locale === 'fr' ? 'Vous' : locale === 'nl' ? 'Jij' : 'You'}
                      </p>
                    ...
                    <div className="bg-accent/10 border border-accent/30 rounded-lg p-2">
                      <div className="flex items-center gap-1 mb-1">
                        ...
                        <p className="text-[9px] font-semibold text-accent">Legal Expert</p>
                      </div>
                      ...
                      <div className="flex items-center gap-1 pt-1.5 border-t border-accent/20">
                        ...
                        <span className="text-[8px] text-accent/70 font-medium">contract.pdf • p.4</span>
                      </div>
                    </div>
                  ...
                  <div className="mt-auto bg-card/80 backdrop-blur-sm border border-border rounded-full px-3 py-2 flex items-center gap-2 shadow-sm">
                    ...
                    <span className="text-[9px] text-muted-foreground">
                      {locale === 'fr' ? 'Posez votre question...' : locale === 'nl' ? 'Stel uw vraag...' : 'Ask your question...'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-16 grid gap-8 lg:grid-cols-3">
            ...
            <div className="rounded-lg border border-border bg-card p-8">
              <h3 className="text-2xl font-semibold">{t('pricing.free.name')}</h3>
              <p className="mt-2 text-muted-foreground">{t('pricing.free.description')}</p>
              <p className="mt-4 text-4xl font-bold">{t('pricing.free.price')}</p>
              <ul className="mt-6 space-y-3">
                {(messages.pricing.free.features as string[]).map((feature: string) => (
              ...
              <Link href="/register" className="mt-8 block w-full rounded-md border border-primary bg-background px-4 py-2 text-center font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition">
                {t('pricing.free.cta')}
              </Link>
            </div>
            ...
            <div className="rounded-lg border-2 border-accent bg-card p-8 shadow-lg">
              <div className="inline-block rounded-full bg-accent px-3 py-1 text-sm font-semibold text-accent-foreground">{t('pricing.pro.badge')}</div>
              <h3 className="mt-4 text-2xl font-semibold">{t('pricing.pro.name')}</h3>
              <p className="mt-2 text-muted-foreground">{t('pricing.pro.description')}</p>
              <p className="mt-4 text-4xl font-bold">{t('pricing.pro.price')}<span className="text-lg font-normal text-muted-foreground">{t('pricing.pro.period')}</span></p>
              <ul className="mt-6 space-y-3">
                {(messages.pricing.pro.features as string[]).map((feature: string) => (
                  <li key={feature} className="flex items-start">
                    <span className="mr-2 text-secondary">✓</span>
                    <span className="text-muted-foreground">{feature}</span>
                  </li>
              ...
              <Link href="/register" className="mt-8 block w-full rounded-md bg-accent px-4 py-2 text-center font-semibold text-accent-foreground hover:opacity-90 transition">
                {t('pricing.pro.cta')}
              </Link>
            </div>
            ...
            <div className="rounded-lg border border-border bg-card p-8">
              ...
              <p className="mt-2 text-muted-foreground">{t('pricing.enterprise.description')}</p>
              <p className="mt-4 text-4xl font-bold">{t('pricing.enterprise.price')}</p>
              <ul className="mt-6 space-y-3">
                {(messages.pricing.enterprise.features as string[]).map((feature: string) => (
                  <li key={feature} className="flex items-start">
                    <span className="mr-2 text-secondary">✓</span>
                    <span className="text-muted-foreground">{feature}</span>
                  </li>
              ...
              <a href="mailto:contact@docuralis.com" className="mt-8 block w-full rounded-md border border-primary bg-background px-4 py-2 text-center font-semibold text-primary hover:bg-primary hover:text-primary-foreground transition">
      ...
      <section className="bg-primary px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl text-center">
          ...
          <div className="mt-10">
            <Link href="/register" className="rounded-md bg-secondary px-8 py-4 text-lg font-semibold text-secondary-foreground hover:opacity-90 transition inline-block">
              {t('cta.button')}
            </Link>
          </div>
        </div>
