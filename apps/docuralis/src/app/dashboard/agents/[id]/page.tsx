...
export default function AgentDetailPage() {
  ...
  const handleActivate = async () => {
    ...
    setActivating(true);
    try {
      const response = await fetch(`/api/collections/${selectedCollectionId}/agents`, {
        method: 'POST',
        ...
        <CardHeader>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <CardDescription>{agent.installCount} installations</CardDescription>
            </div>
          </div>
        </CardHeader>
                    ...
                    return (
                      <div key={action.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Label htmlFor={action.name}>{action.label}</Label>
                          </div>
                        </div>
                        <Switch
                          id={action.name}
                          ...
                          onCheckedChange={(checked) =>
                            setActionState((prev) => ({ ...prev, [action.name]: checked }))
                          }
                        />
                      </div>
                    );
